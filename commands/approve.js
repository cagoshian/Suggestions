...
client.guilds.get(message.guildID).channels.get(db.fetch(`suggestion_${message.guildID}_${sugid}.channel`)).getMessage(db.fetch(`suggestion_${message.guildID}_${sugid}.msgid`)).then(async msg => {
  ...
  if (args.slice(1).join(' ')) {
    if (!db.has(`approvedchannel_${message.guildID}`) || db.fetch(`approvedchannel_${message.guildID}`) == msg.channel.id || !msg.channel.guild.channels.has(db.fetch(`approvedchannel_${message.guildID}`))) {
      msg.edit({embed: {title: `Suggestion #${sugid}`, description: db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`), color: colorToSigned24Bit("#00FF00"), author: {name: `Approved suggestion - ${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).username}#${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).discriminator}`, icon_url: client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).avatarURL || client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).defaultAvatarURL}, footer: {text: client.user.username, icon_url: client.user.avatarURL || client.user.defaultAvatarURL}, fields: [{name: `${message.author.username}'s reason`, value: args.slice(1).join(' ')}], image: {url: db.fetch(`suggestion_${message.guildID}_${sugid}.attachment`)}}})
  ...
  if (db.has(`approvedchannel_${message.guildID}`) && db.fetch(`approvedchannel_${message.guildID}`) != msg.channel.id) {
    ...
    msg.removeReactions()
    if (!db.has(`denydm_${db.fetch(`suggestion_${message.guildID}_${sugid}.author`)}`)) client.users.get(db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).getDMChannel().then(async ch => ch.createMessage({embed: {title: 'Your suggestion has approved!', description: `Your suggestion has approved in \`${message.channel.guild.name}\`.\n**Suggestion:** ${db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`)}\n**Suggestion number:** ${sugid}\n**Reason:** ${args.slice(1).join(' ')}`, color: colorToSigned24Bit("#00FF00")}}))
  }
  if (!args.slice(1).join(' ')) {
    if (!db.has(`approvedchannel_${message.guildID}`) || db.fetch(`approvedchannel_${message.guildID}`) == msg.channel.id || !msg.channel.guild.channels.has(db.fetch(`approvedchannel_${message.guildID}`))) {
  ...
  if (db.has(`approvedchannel_${message.guildID}`) && db.fetch(`approvedchannel_${message.guildID}`) != msg.channel.id) {
    ...
    db.set(`suggestion_${message.guildID}_${sugid}.status`, 'approved')
    msg.removeReactions()
    if (!db.has(`denydm_${db.fetch(`suggestion_${message.guildID}_${sugid}.author`)}`)) client.users.get(db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).getDMChannel().then(async ch => ch.createMessage({embed: {title: 'Your suggestion has approved!', description: `Your suggestion has approved in \`${message.channel.guild.name}\`.\n**Suggestion:** ${db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`)}\n**Suggestion number:** ${sugid}`, color: colorToSigned24Bit("#00FF00")}}))
  }
})
}catch(e){
...
if (dil == "turkish") {
  ...
  client.guilds.get(message.guildID).channels.get(db.fetch(`suggestion_${message.guildID}_${sugid}.channel`)).getMessage(db.fetch(`suggestion_${message.guildID}_${sugid}.msgid`)).then(async msg => {
    ...
    if (args.slice(1).join(' ')) {
      if (!db.has(`approvedchannel_${message.guildID}`) || db.fetch(`approvedchannel_${message.guildID}`) == msg.channel.id || !msg.channel.guild.channels.has(db.fetch(`approvedchannel_${message.guildID}`))) {
        msg.edit({embed: {title: `Öneri #${sugid}`, description: db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`), color: colorToSigned24Bit("#00FF00"), author: {name: `Onaylanmış öneri - ${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).username}#${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).discriminator}`, icon_url: client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).avatarURL || client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).defaultAvatarURL}, footer: {text: client.user.username, icon_url: client.user.avatarURL || client.user.defaultAvatarURL}, fields: [{name: `${message.author.username} adlı yetkilinin cevabı`, value: args.slice(1).join(' ')}], image: {url: db.fetch(`suggestion_${message.guildID}_${sugid}.attachment`)}}})
        db.set(`suggestion_${message.guildID}_${sugid}.channel`, msg.channel.id)
    }
    if (db.has(`approvedchannel_${message.guildID}`) && db.fetch(`approvedchannel_${message.guildID}`) != msg.channel.id) {
        msg.delete()
      ...
      db.set(`suggestion_${message.guildID}_${sugid}.status`, 'approved')
      msg.removeReactions()
      if (!db.has(`denydm_${db.fetch(`suggestion_${message.guildID}_${sugid}.author`)}`)) client.users.get(db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).getDMChannel().then(async ch => ch.createMessage({embed: {title: 'Önerin onaylandı!', description: `Önerin \`${message.channel.guild.name}\` sunucusunda onaylandı.\n**Öneri:** ${db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`)}\n**Öneri numarası:** ${sugid}\n**Sebep:** ${args.slice(1).join(' ')}`, color: colorToSigned24Bit("#00FF00")}}))
    }
    if (!args.slice(1).join(' ')) {
      if (!db.has(`approvedchannel_${message.guildID}`) || db.fetch(`approvedchannel_${message.guildID}`) == msg.channel.id || !msg.channel.guild.channels.has(db.fetch(`approvedchannel_${message.guildID}`))) {
        msg.edit({embed: {title: `Öneri #${sugid}`, description: db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`), color: colorToSigned24Bit("#00FF00"), author: {name: `Onaylanmış öneri - ${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).username}#${client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).discriminator}`, icon_url: client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).avatarURL || client.users.find(u => u.id == db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).defaultAvatarURL}, footer: {text: client.user.username, icon_url: client.user.avatarURL || client.user.defaultAvatarURL}, image: {url: db.fetch(`suggestion_${message.guildID}_${sugid}.attachment`)}}})
    ...
    if (db.has(`approvedchannel_${message.guildID}`) && db.fetch(`approvedchannel_${message.guildID}`) != msg.channel.id) {
      ...
      db.set(`suggestion_${message.guildID}_${sugid}.status`, 'approved')
      msg.removeReactions()
      if (!db.has(`denydm_${db.fetch(`suggestion_${message.guildID}_${sugid}.author`)}`)) client.users.get(db.fetch(`suggestion_${message.guildID}_${sugid}.author`)).getDMChannel().then(async ch => ch.createMessage({embed: {title: 'Önerin onaylandı!', description: `Önerin \`${message.channel.guild.name}\` sunucusunda onaylandı.\n**Öneri:** ${db.fetch(`suggestion_${message.guildID}_${sugid}.suggestion`)}\n**Öneri numarası:** ${sugid}`, color: colorToSigned24Bit("#00FF00")}}))
    }
  })
  }catch(e){
