...
let dil = db.fetch(`dil_${message.guildID}`) || "english";
  ...
  if (dil == "english") {
    ...
    if (!db.has(`botcekilis`)) message.channel.createMessage({embed: {title: '__**Public Commands**__', description: helpcommandsmap, color: colorToSigned24Bit("#2F3136"), footer: {text: client.user.username, icon_url: client.user.avatarURL || client.user.defaultAvatarURL}}})
  ...
  if (dil == "turkish") {
    ...
    if (!db.has(`botcekilis`)) message.channel.createMessage({embed: {title: '__**Herkese Açık Komutlar**__', description: helpcommandsmap, color: colorToSigned24Bit("#2F3136"), footer: {text: client.user.username, icon_url: client.user.avatarURL || client.user.defaultAvatarURL}}})
